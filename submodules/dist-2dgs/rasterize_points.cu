	...
	const bool debug)
{
	...
	torch::Tensor cov3D = torch::full({P, 6}, 0, means3D.options().dtype(torch::kFloat32));
...
PreprocessGaussiansCUDA(
	...
	torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
	// of shape (P, 6)
	torch::Tensor transMat = torch::full({P, 9}, 0.0, means3D.options());
	// of shape (P, 4)
	torch::Tensor normal_opacity = torch::full({P, 4}, 0.0, means3D.options());
	// of shape (P, 3)
	...
	if(P != 0)
	{
		...
		rendered = CudaRasterizer::Rasterizer::preprocessForward(
			...
			transMat.contiguous().data<float>(),
			rgb.contiguous().data<float>(),
			reinterpret_cast<float4*>(normal_opacity.contiguous().data<float>()),
			clamped.contiguous().data<bool>(),
			debug,
			args);
	}
	return std::make_tuple(rendered, means2D, depths, radii, transMat, normal_opacity, rgb, clamped);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
  PreprocessGaussiansBackwardCUDA(
	...
	torch::Tensor& dL_dmeans2D,// (P, 3)
	const torch::Tensor& dL_dnormal_opacity,
	...
	const pybind11::dict &args)
{
  ...
  // set dL_dopacity[..., 0] = dL_dnormal_opacity[..., 3]
  dL_dnormal.select(1, 0).copy_(dL_dnormal_opacity.select(1, 0));
  dL_dnormal.select(1, 1).copy_(dL_dnormal_opacity.select(1, 1));
  dL_dnormal.select(1, 2).copy_(dL_dnormal_opacity.select(1, 2));
  dL_dnormal = dL_dnormal.contiguous();

  dL_dopacity.select(1, 0).copy_(dL_dnormal_opacity.select(1, 3));
...
//   torch::Tensor dL_dtransMat = torch::zeros({P, 9}, means3D.options());
  ...
  return std::make_tuple(dL_dmeans3D, dL_dmeans2D, dL_dscales, dL_drotations, dL_dsh, dL_dopacity);
}
...
RenderGaussiansCUDA(
	...
	torch::Tensor& depths,
	torch::Tensor& radii,
	torch::Tensor& normal_opacity,
	torch::Tensor& rgb,//3dgs intermediate results
	torch::Tensor& transMat,
...
//   torch::Tensor n_contrib = torch::full({tile_num}, 0, int_opts);
  ...
  return std::make_tuple(rendered, out_color, out_others, geomBuffer, binningBuffer, imgBuffer, accum_max_count, transmittance, num_occluder);
}
	...
	const pybind11::dict &args)
{
  ...
  torch::Tensor dL_dnormal = torch::zeros({P, 3}, means2D.options());//The requires_grad property for the gradient tensor is typically False
  torch::Tensor dL_dopacity = torch::zeros({P, 1}, means2D.options());
  torch::Tensor dL_dtransMat = torch::zeros({P, 9}, means2D.options());
  ...
  torch::Tensor dL_dnormal_opacity = torch::zeros({P, 4}, means2D.options());
  // set dL_dnormal_opacity[..., 0] = dL_dnormal[..., 0, 0]
  dL_dnormal_opacity.select(1, 0).copy_(dL_dnormal.select(1, 0));
  // set dL_dnormal_opacity[..., 1] = dL_dnormal[..., 0, 1]
  dL_dnormal_opacity.select(1, 1).copy_(dL_dnormal.select(1, 1));
  // set dL_dnormal_opacity[..., 2] = dL_dnormal[..., 1, 1]
  dL_dnormal_opacity.select(1, 2).copy_(dL_dnormal.select(1, 2));
  // set dL_dnormal_opacity[..., 3] = dL_dopacity[..., 0]
  dL_dnormal_opacity.select(1, 3).copy_(dL_dopacity.select(1, 0));
  
  //TODO: in pytorch, when the reference to a tensor decreases to 0, the memory will be freed.
  //But what will happen to libtorch?
  return std::make_tuple(dL_dmeans2D, dL_dnormal_opacity, dL_dcolors, dL_dtransMat);
}
