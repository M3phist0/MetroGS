...
// Forward method for converting the input spherical 
...
renderCUDA(
	...
	float* __restrict__ accum_count,
	...
	bool record_transmittance)
{
	...
	if (compute_locally != nullptr && !compute_locally[block_id])
		return;
	...
	if (idx_max != -1 && idx_max == idx_mid) {
		atomicAdd(&(accum_count[idx_max]), 1);
	}
	...
	// } 
	// else if (idx_mid != -1) {
	// 	atomicAdd(&(accum_count[idx_mid]), 1);
	// }
