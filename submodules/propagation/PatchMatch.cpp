...
void PatchMatch::CudaSpaceInitialization()
{
    ...
    if (normals.size() > 0) {
        cudaMalloc((void**)&normals_cuda, sizeof(float3) * (cameras[0].height * cameras[0].width));
        cudaMemcpy(normals_cuda, normals[0].data_ptr<float>(), sizeof(float3) * cameras[0].height * cameras[0].width, cudaMemcpyHostToDevice);
    } else {
...
void PatchMatch::SetRadiusIncrement(int radius_increment) {
    params.radius_increment = radius_increment;
}
...
void PatchMatch::SetMaxScale(int max_scale)
{
