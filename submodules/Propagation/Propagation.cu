...
__device__ float4 GenerateInitPlaneHypothesis(const Camera camera, const int2 p, curandState *rand_state, const float depth_min, const float depth_max, float init_depth, float3 init_normal)
{
    float depth = init_depth;
    if (depth <= 0){
        depth = curand_uniform(rand_state) * (depth_max - depth_min) + depth_min;
    ...
    plane_hypothesis.x = init_normal.x;
    plane_hypothesis.y = init_normal.y;
    plane_hypothesis.z = init_normal.z;
    plane_hypothesis.w = GetDistance2Origin(camera, p, depth, plane_hypothesis);
    return plane_hypothesis;
}
...
__device__ float ComputeBilateralNCC(const cudaTextureObject_t ref_image, const Camera ref_camera, const cudaTextureObject_t src_image, const Camera src_camera, const int2 p, const float4 plane_hypothesis, const PatchMatchParams params, const int scale)
{
    const float cost_max = 2.0f;
    int nSizeStep = params.radius_increment;
    ...
    if (patch_size / 2 > radius) {
        radius = patch_size / 2;
        nSizeStep = params.radius_increment;
    }
...
__device__ float ComputeMultiViewInitialCostandSelectedViews(const cudaTextureObject_t *images, const Camera *cameras, const int2 p, const float4 plane_hypothesis, unsigned int *selected_views, const PatchMatchParams params, const int scale)
{
    float cost_max = 2.0f;
    ...
    for (int i = 1; i < params.num_images; ++i) {
        float c = ComputeBilateralNCC(images[0], cameras[0], images[i], cameras[i], p, plane_hypothesis, params, scale);
        cost_vector[i - 1] = c;
...
__device__ void ComputeMultiViewCostVector(const cudaTextureObject_t *images, const Camera *cameras, const int2 p, const float4 plane_hypothesis, float *cost_vector, const PatchMatchParams params, const int scale)
{
    for (int i = 1; i < params.num_images; ++i) {
        cost_vector[i - 1] = ComputeBilateralNCC(images[0], cameras[0], images[i], cameras[i], p, plane_hypothesis, params, scale);
    }
}
...
__global__ void RandomInitialization(cudaTextureObjects *texture_objects, Camera *cameras, float4 *plane_hypotheses, 
                                     ...
                                     const PatchMatchParams params, float *depths, float3 *normals, const int scale)
{
    ...
    if (!params.geom_consistency) {
        if (normals != nullptr) {
           plane_hypotheses[center] = GenerateInitPlaneHypothesis(cameras[0], p, &rand_states[center], params.depth_min, params.depth_max, depths[center], normals[center]);
        } else {
        ...
        costs[center] = ComputeMultiViewInitialCostandSelectedViews(texture_objects[0].images, cameras, p, plane_hypotheses[center], &selected_views[center], params, scale);
    }
    else {
...
__device__ void CheckerboardPropagation(const cudaTextureObject_t *images, const cudaTextureObject_t *depths, const Camera *cameras, 
                                        ...
                                        const int2 p, const PatchMatchParams params, const int iter, const int scale)
{
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
    ...
    int up_far = center - 5 * width;
    int down_near = center + width;
    int down_far = center + 5 * width;
    ...
    if (p.y > 4) {
        ...
        for (int i = 1; i < 11; ++i) {
            if (p.y > 4 + 4 * i) {
                int pointTemp = up_far - 4 * i * width;
        ...
        up_far = costMinPoint;
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_far], cost_array[1], params, scale);
    }
    ...
    if (p.y < height - 5) {
        ...
        for (int i = 1; i < 11; ++i) {
            if (p.y < height - 5 - 4 * i) {
                int pointTemp = down_far + 4 * i * width;
                if (costs[pointTemp] < costMin) {
                    costMin = costs[pointTemp];
                    costMinPoint = pointTemp;
                }
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_far], cost_array[3], params, scale);
    }
    ...
    if (p.x > 4) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_far], cost_array[5], params, scale);
    }
    ...
    if (p.x < width - 5) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_far], cost_array[7], params, scale);
    }
    ...
    if (p.y > 0) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_near], cost_array[0], params, scale);
    }
    ...
    if (p.y < height - 1) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_near], cost_array[2], params, scale);
    }
    ...
    if (p.x > 0) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_near], cost_array[4], params, scale);
    }
    ...
    if (p.x < width - 1) {
        ...
        ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_near], cost_array[6], params, scale);
    }
    ...
    ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[center], cost_vector_now, params, scale);
    float cost_now = 0.0f;
    for (int i = 0; i < params.num_images - 1; ++i) {
        if (params.geom_consistency) {
    ...
    PlaneHypothesisRefinement(images, depths, cameras, &plane_hypotheses[center], &depth_now, &costs[center], &rand_states[center], view_weights, weight_norm, p, params, scale);
}
...
__global__ void BlackPixelUpdate(cudaTextureObjects *texture_objects, cudaTextureObjects *texture_depths, 
                                 ...
                                 unsigned int *selected_views, const PatchMatchParams params, const int iter, const int scale)
{
    ...
    CheckerboardPropagation(texture_objects[0].images, texture_depths[0].images, cameras, plane_hypotheses, costs, rand_states, selected_views, p, params, iter, scale);
}
...
void PatchMatch::RunPatchMatch()
{
    ...
    int max_scale = params.max_scale;

    RandomInitialization<<<grid_size_randinit, block_size_randinit>>>(texture_objects_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, rand_states_cuda, selected_views_cuda, params, depths_cuda, normals_cuda, max_scale); 
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
    
    for (int scale = max_scale; scale >= 0; scale--) {
        for (int i = 0; i < max_iterations; ++i) {
            BlackPixelUpdate<<<grid_size_checkerboard, block_size_checkerboard>>>(texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, rand_states_cuda, selected_views_cuda, params, i, scale);
            CUDA_SAFE_CALL(cudaDeviceSynchronize());
            
            RedPixelUpdate<<<grid_size_checkerboard, block_size_checkerboard>>>(texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, rand_states_cuda, selected_views_cuda, params, i, scale);
            CUDA_SAFE_CALL(cudaDeviceSynchronize());
        }
    }
...
torch::Tensor propagate_cuda(torch::Tensor images, torch::Tensor intrinsics, torch::Tensor poses, 
                            torch::Tensor depth, torch::Tensor normal, torch::Tensor depth_intervals, 
                            int patch_size, int max_scale, int radius_increment)
{
    // cudaSetDevice(0);

    PatchMatch pm;
    pm.SetRadiusIncrement(radius_increment);
    pm.SetPatchSize(patch_size);
    pm.SetMaxScale(max_scale);
