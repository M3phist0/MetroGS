...
if args.depth_dir is None:
    args.depth_dir = os.path.join(args.dataset_dir, "estimated_mask_depths")
if args.output is None:
    args.output = os.path.join(args.dataset_dir, "estimated_mask_depth_scales.json")
...
def get_scales(key, cameras, images, points3d_ordered, points3d_error_ordered, args):
    ...
    data = np.load(os.path.join(args.depth_dir, "{}.npy".format(image_meta.name)))  # already normalized
    invmonodepthmap = data[..., 0].astype(np.float32)
    maskmap = data[..., 1].astype(np.uint8)

    # if maskmap.sum() != maskmap.shape[0] * maskmap.shape[1]:

    # print(invmonodepthmap[~maskmap])
    ...
    if valid.sum() > 10 and (invcolmapdepth.max() - invcolmapdepth.min()) > 1e-3:
        ...
        mask = cv2.remap(maskmap, maps[..., 0], maps[..., 1], interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REPLICATE)[..., 0].astype(np.bool)

        invcolmapdepth = invcolmapdepth[mask]
        invmonodepth = invmonodepth[mask]
