...
class GaussianModelLoader:
    ...
    def initialize_model_from_checkpoint(cls, checkpoint: dict, device):
        ...
        if isinstance(hparams["gaussian"], Gaussian):
            ...
            if "gaussians.anchor" in model_state_dict:
                from internal.models.anchor_gaussian import AnchorGaussian
                model = AnchorGaussian().instantiate()
            elif "gaussians.appearance_features" in model_state_dict:
        ...
        if "gaussians.anchor" in model_state_dict:
            model.setup_from_number(model_state_dict["gaussians.anchor"].shape[0])
        else:
