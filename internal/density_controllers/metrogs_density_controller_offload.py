from .vanilla_density_controller_offload import VanillaDensityController, VanillaDensityControllerImpl, build_rotation, VanillaGaussianModel, List, LightningModule
    ...
    def update_states(self, outputs):
        ...
        for outputs in outputs_list:
            if outputs['render'] is None:
                continue
            viewspace_point_tensor, visibility_filter, radii = outputs["viewspace_points"], outputs["visibility_filter"].cpu(), outputs["radii"].cpu()
            # retrieve viewspace_points_grad_scale if provided
            viewspace_points_grad_scale = outputs.get("viewspace_points_grad_scale", None)
            acc_pix = outputs["acc_pix"].cpu()
