...
from internal.optimizers_offload import OptimizerConfig, Adam, CPUAdam, SelectiveAdam, SparseGaussianAdam
...
class OptimizationConfig:
    ...
    optimizer: OptimizerConfig = field(default_factory=lambda: {"class_path": "CPUAdam"})

class VanillaGaussianGPUProxy:
    def __init__(self, cpu_model, device, non_blocking=True):
        self.cpu = cpu_model
        self.device = device
        self.non_blocking = non_blocking
        self._cache = {}

    def _to_gpu(self, key, t):
        out = self._cache.get(key, None)
        if out is None or out.device != self.device:
            out = t.to(self.device, non_blocking=self.non_blocking)
            self._cache[key] = out
        return out

    @property
    def active_sh_degree(self):
        return self.cpu.active_sh_degree

    @property
    def get_xyz(self):
        return self._to_gpu("means", self.cpu.gaussians["means"])

    @property
    def get_scaling(self):
        scales = self._to_gpu("scales_raw", self.cpu.gaussians["scales"])
        return torch.exp(scales)

    @property
    def get_rotation(self):
        rots = self._to_gpu("rot_raw", self.cpu.gaussians["rotations"])
        return torch.nn.functional.normalize(rots)

    @property
    def get_features(self):
        shs_dc = self._to_gpu("shs_dc", self.cpu.gaussians["shs_dc"])
        shs_rest = self._to_gpu("shs_rest", self.cpu.gaussians["shs_rest"])
        return torch.cat([shs_dc, shs_rest], dim=1)

    @property
    def get_opacity(self):
        op = self._to_gpu("opacity_raw", self.cpu.gaussians["opacities"])
...
class VanillaGaussianModel(
    ...
    def setup_from_pcd(self, xyz: Union[torch.Tensor, np.ndarray], rgb: Union[torch.Tensor, np.ndarray], *args, **kwargs):
        ...
        dist2 = torch.clamp_min(distCUDA2(fused_point_cloud.cuda()), 0.0000001).to(fused_point_cloud.device).cpu()
        ...
        means = nn.Parameter(fused_point_cloud.pin_memory().requires_grad_(True))
        shs_dc = nn.Parameter(shs[:, :, 0:1].transpose(1, 2).contiguous().pin_memory().requires_grad_(True))
        shs_rest = shs[:, :, 1:].transpose(1, 2).contiguous()
        shs_rest = nn.Parameter(shs_rest.pin_memory().requires_grad_(True))

        scales = nn.Parameter(scales.pin_memory().requires_grad_(True))
        rotations = nn.Parameter(rots.pin_memory().requires_grad_(True))
        opacities = nn.Parameter(opacities.pin_memory().requires_grad_(True))
    ...
    def setup_from_number(self, n: int, *args, **kwargs):
        ...
        means = nn.Parameter(means.pin_memory().requires_grad_(True))
        shs_dc = nn.Parameter(shs_dc.pin_memory().requires_grad_(True))
        shs_rest = nn.Parameter(shs_rest.pin_memory().requires_grad_(True))
        scales = nn.Parameter(scales.pin_memory().requires_grad_(True))
        rotations = nn.Parameter(rotations.pin_memory().requires_grad_(True))
        opacities = nn.Parameter(opacities.pin_memory().requires_grad_(True))
    ...
    def get_gpu_proxy(self, device="cuda", non_blocking=True):
        return VanillaGaussianGPUProxy(self, device, non_blocking)