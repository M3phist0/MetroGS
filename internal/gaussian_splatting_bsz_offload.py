...
class GaussianSplatting(LightningModule):
    ...
    # offload needed
    def _apply(self, fn):
        # 临时把 gaussian_model 从 module tree 里摘掉，避免 super()._apply 递归进去
        gm = self._modules.get("gaussian_model", None)
        if gm is not None:
            self._modules["gaussian_model"] = None

        try:
            ret = super()._apply(fn)   # 这会把其它模块/buffer/param 搬到 GPU
        finally:
            # 放回 gaussian_model（它保持原设备：CPU）
            if gm is not None:
                self._modules["gaussian_model"] = gm
    ...
    def log_metrics(
        ...
        for name in metrics:
            self.log(
                ...
                metrics[name]/(denom if name != 'gs' else 1),
    ...
    def setup(self, stage: str):
        if stage == "fit":
            print("precision", self.trainer.precision)
            if self.hparams["initialize_from"] is None:
                ...
                print("setup fit self.gaussian_model device:", self.gaussian_model.get_xyz.device)
            else:
    ...
    def transfer_batch_to_device(self, batch: Any, device: torch.device, dataloader_idx: int) -> Any:
        ...
        for item in batch:
            ...
            item_list.append((camera, (image_name, gt_image, masked_pixels), extra_data))

        return item_list

    def forward(self, camera_list, **kwargs):
        if self.training is True:
            return self.renderer.training_forward(
                self.trainer.global_step * self.batch_size,
                self,
                camera_list,
                self.gaussian_model.get_gpu_proxy(), # offload needed
                bg_color=self.get_background_color().to(camera_list[0].R.device),
        ...
        return self.renderer(
            camera_list,
            self.gaussian_model.get_gpu_proxy(), # offload needed
            bg_color=self._fixed_background_color().to(camera_list[0].R.device),
            render_types=self.renderer_output_types,
            **kwargs
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.trainer.world_size > 1:
            ...
            if self.trainer.global_rank > 0:
                for i in range(len(batch)):
                    camera, image_info, extra_data = batch[i]
                    if broadcast_extra:
                        image_info = (image_info[0], images_to_broadcast[i][:3], image_info[2])
                        if images_to_broadcast[i][4].sum() > 0:
                            extra_data = (images_to_broadcast[i][3], images_to_broadcast[i][4])
                        else:
                            extra_data = None
                    else:
                        image_info = (image_info[0], images_to_broadcast[i], image_info[2])
                    batch[i] = (camera, image_info, extra_data)

        global_step = self.trainer.global_step  # must start from 1 to prevent densify at the beginning
        iteration = self.trainer.global_step * bsz
        # print("global_step:", self.trainer.global_step, self.trainer.max_steps, iteration)
        ...
        if any([iteration <= save_iteration < iteration + bsz for save_iteration in self.hparams["save_iterations"]]) \
            and self.is_final_step(global_step) is False and self.trainer.global_step != self.restored_global_step:
            self.save_gaussians()

        # call renderer hook
        self.renderer.before_training_step(iteration, self)

        # forward
        outputs_list = self(camera_list)
        # metrics
        ...
        if "extra_loss" in metrics.keys():
            ...
            for outputs in outputs_list:
                ...
                grad_norm_avg_list.append(grad_norm_avg)
                org_grad_list.append(org_grad)

            self.manual_backward(metrics["extra_loss"])

            for idx, outputs in enumerate(outputs_list):
                grad_norm_avg = grad_norm_avg_list[idx]
                org_grad = org_grad_list[idx]
        ...
        for key in self.gaussian_model.gaussians:
            if self.gaussian_model.gaussians[key].grad is not None:
                self.gaussian_model.gaussians[key].grad /= bsz
    ...
    def on_train_batch_end(self, outputs: STEP_OUTPUT, batch: Any, batch_idx: int) -> None:
        ...
        iteration = global_step * self.batch_size

        self.gaussian_model.on_train_batch_end(iteration, self)

        self.renderer.after_training_step(iteration, self)

        self.light_gaussian_prune(iteration)

        for i in self.on_train_batch_end_hooks:
            i(outputs, batch, self.gaussian_model, iteration, self)
    ...
    def validation_step(self, batch, batch_idx, name: str = "val"):
        ...
        bsz = len(batch)
        camera_list = [item[0] for item in batch]
        ...
        if self.hparams["test_speed"]:
            ...
            metrics["time(s)"] = torch.tensor((end - start) / bsz)
        else:
            outputs_list = self(camera_list)
        ...
        if self.trainer.world_size > 1:
            for item, outputs in zip(batch, outputs_list):
                ...
                if outputs["render"] is None:
                    outputs["render"] = torch.zeros(gt_image.shape, device="cuda", dtype=torch.float32)
                    outputs["rend_dist"] = torch.zeros((1, gt_image.shape[1], gt_image.shape[2]), device="cuda", dtype=torch.float32)
                    outputs["surf_depth"] = torch.zeros((1, gt_image.shape[1], gt_image.shape[2]), device="cuda", dtype=torch.float32)
                    outputs["rend_normal"] = torch.zeros((3, gt_image.shape[1], gt_image.shape[2]), device="cuda", dtype=torch.float32)
                    outputs["surf_normal"] = torch.zeros((3, gt_image.shape[1], gt_image.shape[2]), device="cuda", dtype=torch.float32)
                
                image = outputs["render"].contiguous()
                dist = outputs["rend_dist"].contiguous()
                surf_depth = outputs["surf_depth"].contiguous()
                rend_normal = outputs["rend_normal"].contiguous()
                surf_normal = outputs["surf_normal"].contiguous()

                torch.distributed.all_reduce(image, op=torch.distributed.ReduceOp.SUM)
                torch.distributed.all_reduce(dist, op=torch.distributed.ReduceOp.SUM)
                torch.distributed.all_reduce(surf_depth, op=torch.distributed.ReduceOp.SUM)
                torch.distributed.all_reduce(rend_normal, op=torch.distributed.ReduceOp.SUM)
                torch.distributed.all_reduce(surf_normal, op=torch.distributed.ReduceOp.SUM)

                outputs["render"] = image
                outputs["rend_dist"] = dist
                outputs["surf_depth"] = surf_depth
                outputs["rend_normal"] = rend_normal
                outputs["surf_normal"] = surf_normal
        
        metrics, prog_bar = self.metric.get_validate_metrics(self, self.gaussian_model, batch, outputs_list)
            ...
            if self.trainer.global_rank == 0 and self.hparams["save_val_output"] is True and (
                ...
                image_output_path = os.path.join(
                    ...
                    "epoch={}-step={}".format(
                        ...
                        max(self.trainer.global_step * self.batch_size, self.restored_global_step),
                    ),
                ...
                depth_output_path = os.path.join(
                    ...
                    "{}_depth.png".format(image_info[0].replace("/", "_"))
                )
                depth = outputs["surf_depth"]
                depth = 1. / depth.detach().cpu().numpy().squeeze(0)
                depth[0, 0] = 0.0
        ...
        if self.batch_size != 1:
            for optimizer in optimizers:
                for param_group in optimizer.param_groups:
                    # param_group["lr"] *= self.batch_size
                    lr_scale = math.sqrt(self.batch_size)
                    param_group["lr"] *= lr_scale
                    if "eps" in param_group:  # Adam
                        param_group["eps"] /= lr_scale
                        param_group["betas"] = [beta**self.batch_size for beta in param_group["betas"]]
    ...
    def on_train_end(self):
        ...
        for opt in optimizers:
            # 核心判断：只有包含 destroy 属性且可调用的对象才执行
            # 这样可以完美兼容原生 torch 优化器和你的 CPUAdam
            destroy_method = getattr(opt, "destroy", None)
            if callable(destroy_method):
                try:
                    destroy_method()
                    # 打印一下 ID 方便调试，确认 C++ 侧 Key 已释放
                    print(f"DEBUG: Successfully destroyed Optimizer ID: {getattr(opt, 'opt_id', 'unknown')}")
                except Exception as e:
                    print(f"WARNING: Error during optimizer destruction: {e}")
    ...
    def save_gaussians(self):
        is_mp_strategy = isinstance(self.trainer.strategy, internal.mp_strategy_offload.MPStrategy)
        ...
        if self.hparams["save_ply"] is True:
            ...
            with torch.no_grad():
                output_dir = os.path.join(self.hparams["output_path"], "point_cloud",
                                          "iteration_{}".format(self.trainer.global_step * self.batch_size))
        ...
        checkpoint_path = os.path.join(
            self.hparams["output_path"],
            "checkpoints",
            "epoch={}-step={}{}.ckpt".format(self.trainer.current_epoch, self.trainer.global_step * self.batch_size, checkpoint_name_suffix),
        )
        ...
        with torch.no_grad():
            ...
            store_ply(os.path.join(
                self.hparams["output_path"],
                "checkpoints",
                "epoch={}-step={}{}-xyz_rgb.ply".format(self.trainer.current_epoch, self.trainer.global_step * self.batch_size, checkpoint_name_suffix),
