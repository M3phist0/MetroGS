...
class AnchorRenderer(Renderer):
    ...
    def generate_neural_gaussians(self, viewpoint_camera, pc : AnchorGaussianModel, visible_mask=None, is_training=False):
        ...
        if pc.get_scaling.shape[1] == 5:
            scaling_repeat, repeat_anchor, color, scale_rot, offsets = masked.split([5, 3, 3, 7, 3], dim=-1) # 2DGS
        else:
    ...
    def forward(
        ...
        visible_mask = self.prefilter_voxel(viewpoint_camera, pc, bg_color, scaling_modifier)
        if visible_mask is None:
            visible_mask = torch.ones(pc.get_anchor.shape[0], dtype=torch.bool, device = pc.get_anchor.device)
        ...
        raster_settings = AnchorSettings(
            image_height=int(viewpoint_camera.height),
            image_width=int(viewpoint_camera.width),
            tanfovx=tanfovx,
            tanfovy=tanfovy,
        ...
        rasterizer = AnchorRasterizer(raster_settings=raster_settings)
        ...
        rendered_image, radii, invdepth = output
