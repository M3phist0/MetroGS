...
class DistributedRendererImpl(Renderer):
    ...
    def forward_eval(
        ...
        if self.use_app:
            ...
            # print("xyz:", means3D.shape, viewpoint_camera.appearance_id, self.model.aabb)
            # self.model.aabb = torch.tensor(
            #     [-5.8394694328308105, -2.958742618560791, -0.9381375312805176, 2.6365408897399902, 4.0408806800842285, 5.501363277435303]
            #                                ).cuda()
...
# def find_most_similar_cameras_robust(train_cameras, val_cameras, point_cloud_xyz, alpha = 0.7, k_overlap = 10):
#     train_centers = torch.stack([cam.camera_center for cam in train_cameras])  # (N_train, 3)
#     train_Rs = [cam.R for cam in train_cameras]
#     results = []
...
#     train_visible_indices_list = []
#     # print("Preprocessing: Calculating visible point indices for all training cameras...")
    
#     N_points = point_cloud_xyz.shape[0]
#     N_train = len(train_cameras)
#     target_device = train_cameras[0].device
#     print("finding device:", target_device)
#     M_train = torch.zeros((N_train, N_points), dtype=torch.bool, device=target_device)
#     for i, train_cam in enumerate(train_cameras):
#         # project_points 必须只返回在视锥内的点索引
#         _, train_indices = project_points(train_cam, point_cloud_xyz)
#         train_visible_indices_list.append(train_indices)

#         M_train[i, train_indices] = True

#     for val_idx, val_cam in enumerate(val_cameras):
#         # print(f"{val_idx} / {len(val_cameras)}")
#         val_pixels, val_indices = project_points(val_cam, point_cloud_xyz)
#         val_visible_indices = val_indices # (N_val_subset,)
#         N_subset_val = val_visible_indices.shape[0]
...
#         overlap_scores_mask = M_train[:, val_indices] # (N_train, N_subset_val)
        
#         overlap_counts = torch.sum(overlap_scores_mask.int(), dim=1) # (N_train,)

#         # max_overlap_count = torch.max(overlap_counts).item()
#         # best_train_local_idx = torch.argmax(overlap_counts).item()
#         # similarity_score = max_overlap_count / N_subset_val
#         # results.append((val_idx, best_train_local_idx, similarity_score))

#         current_k = min(k_overlap, N_train)
        
#         # 找到重叠点数最大的前 K 个值和它们在 train_cameras 列表中的局部索引
#         topk_counts, candidate_indices = torch.topk(overlap_counts, k=current_k, largest=True) 

#         # 检查重叠点数最大的相机是否为 0，如果是则跳过 (意味着没有共同点)
#         if topk_counts[0].item() == 0:
#              results.append((val_idx, -1, float('inf')))
#              continue

#         cand_centers = train_centers[candidate_indices]
#         cand_Rs = [train_Rs[i.item()] for i in candidate_indices]
...
def get_dis(target, candi, L):
    ...
    # def get_first_diff_pos(target: str, s: str) -> int:
    #     for idx in range(len(target)):
    #         if target[idx] != s[idx]:
    #             return idx
    #     return -1  # 所有字符完全一致
    # pos = get_first_diff_pos(target, candi)
    
    # if pos == -1:
    #     return (-1, 0)
    
    # char_dist = abs(ord(target[pos]) - ord(candi[pos]))
    # return (L - pos, char_dist)
