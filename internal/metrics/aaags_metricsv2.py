...
class DistributedMetricsModule(CityGSV2MetricsModule):
    ...
    def get_train_metrics(self, pl_module, gaussian_model, step: int, batch, outputs) -> Tuple[Dict[str, Any], Dict[str, bool]]:
        ...
        if use_multi_view and step >= self.config.multi_view_from:
            with torch.no_grad():
                ...
                if need_propagate:
                    ...
                    # ed = time.time()
                    # print("time:", ed - st, pl_module.trainer.global_rank)
        ...
        for i, (item, output) in enumerate(zip(batch, outputs)):
            ...
            # ssim_metric = self.ssim(appearance, gt_image)
            ...
            if step < self.config.single_view_from or gt_inverse_depth is None or (use_multi_view and step >= self.config.multi_view_from):
                ...
                # n_reg = torch.tensor(0., device=camera.device)
            else:
                # if self.config.normal_factor > 0:
                #     if isinstance(gt_inverse_depth, tuple):
                #         gt_normal = pl_module.renderer.depth_to_normal(camera, 1. / gt_inverse_depth[0].unsqueeze(0)).permute(2, 0, 1)
                #         n_reg = self.config.normal_factor * d_reg_weight \
                #             * (1 - (rend_normal * surf_normal * gt_inverse_depth[1].unsqueeze(0)).sum(dim=0))[None].mean()
                #     else:
                #         gt_normal = pl_module.renderer.depth_to_normal(camera, 1. / gt_inverse_depth.unsqueeze(0)).permute(2, 0, 1)
                #         n_reg = self.config.normal_factor * d_reg_weight \
                #             * (1 - (rend_normal * surf_normal).sum(dim=0))[None].mean()
                
                # # print("shape:", gt_normal.shape, output["surf_depth"].shape, output["surf_normal"].shape, output["rend_normal"].shape)
                # if '000' in image_name:
                    # normal_i = gt_normal.permute(1,2,0)
                    ...
                    # cv2.imwrite(os.path.join("./tmt/",  f"{image_name}_gtn.png"), normal_i)
            ...
            # if mapping is not None:
            #     map_reg = self.config.lambda_mapping * ((mapping - 1.0).abs()).mean()
            #     metrics_i["loss"] = metrics_i["loss"] + map_reg
            ...
            if step >= self.config.scale_reg_from:
                ...
                # s_reg = self.config.lambda_scale * sorted_scale[:, -1].mean()
                ...
                if need_propagate:
                    with torch.no_grad():
                        ...
                        if src_gt_image_list is not None:
                            ...
                            # if '00000' in image_name:
                            #     depth = inv_propagated_depth.detach().squeeze().cpu().numpy()
                            #     depth_i = (depth - depth.min()) / (depth.max() - depth.min() + 1e-8)
                            #     depth_i = (depth_i * 255).clip(0, 255).astype(np.uint8)
                            #     depth_color = cv2.applyColorMap(depth_i, cv2.COLORMAP_JET)
                            #     cv2.imwrite(os.path.join("./tmt/",  f"{image_name}_mvs.png"), depth_color)
            ...
            if 'DJI_202312191150' in image_name:
                os.makedirs("./tmt/", exist_ok=True)
                depth = 1. / output["surf_depth"].detach().squeeze().cpu().numpy()
                depth_i = (depth - depth.min()) / (depth.max() - depth.min() + 1e-8)
                depth_i = (depth_i * 255).clip(0, 255).astype(np.uint8)
                depth_color = cv2.applyColorMap(depth_i, cv2.COLORMAP_JET)
                cv2.imwrite(os.path.join("./tmt/",  f"{image_name}_depth.png"), depth_color)

                normal_i = surf_normal.permute(1,2,0)
                normal_i = normal_i/(normal_i.norm(dim=-1, keepdim=True)+1.0e-8)
                normal_i = normal_i.detach().cpu().numpy()
                normal_i = ((normal_i+1) * 127.5).astype(np.uint8).clip(0, 255)
                cv2.imwrite(os.path.join("./tmt/",  f"{image_name}_normal.png"), normal_i)

                torchvision.utils.save_image(image, os.path.join("./tmt/",  f"{image_name}.png"))
    ...
    def get_data_weights(self, pl_module, batch, outputs, data_weights):
        ...
        with torch.no_grad():
            for item, output in zip(batch, outputs):
                ...
                weight = torch.exp(self.config.sample_alpha * (1.0 - self.ssim(image, gt_image)))
