...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        print(f"load deivce, cam:{camera_device}, img:{image_device}")

        if self.use_multi_view:
            multi_view_path = os.path.join(
                os.path.dirname(os.path.dirname(image_set.image_paths[0])),
                "multi_view.json"
            )
            print("load multi-view info from:", multi_view_path)
            with open(multi_view_path, 'r') as f:
                self.multi_view_info = json.load(f)
            self.name2index = {value: idx for (idx, value) in enumerate(self.image_set.image_names)}
    ...
    def get_multi_view_data(self, index):
        ref_name = self.image_set.image_names[index]
        src_name_list = self.multi_view_info[ref_name]
        if len(src_name_list) == 0:
            return self.image_cameras[index], ref_name, None

        # src_name = random.choice(src_name_list)
        # src_index = self.name2index[src_name]
        # _, image, _ = self.get_image(src_index)

        # return self.image_cameras[src_index], src_name, image
        ...
        for idx, src_name in enumerate(src_name_list):
            ...
            src_index = self.name2index[src_name]
            _, image, _ = self.get_image(src_index, is_src=True)

            src_camera_list.append(self.image_cameras[src_index])
            _src_name_list.append(src_name)
            src_image_list.append(image)
                
        return src_camera_list, _src_name_list, src_image_list
    
    def get_multi_view_data_index(self, index, N=3, mv_shuffle=True):
        ref_name = self.image_set.image_names[index]
        if ref_name not in self.multi_view_info:
            src_name_list = []
            print(f"Warning: {ref_name} have not multi-view info.")
        else:
            src_name_list = self.multi_view_info[ref_name]
        
        if len(src_name_list) > 0 and mv_shuffle:
            random.shuffle(src_name_list)

        src_index_list = []
        for idx, src_name in enumerate(src_name_list):
            if len(src_index_list) >= N:
                break
            if src_name in self.name2index:
                src_index = self.name2index[src_name]
                src_index_list.append(src_index)

        while len(src_index_list) < N:
            src_index_list.append(index)
                
        return src_index_list
    ...
    def get_extra_data(self, index):
        if self.global_rank <= 0:
            extra_data = self.image_set.extra_data_processor(self.image_set.extra_data[index])
        else:
            extra_data = None
        # if self.use_multi_view:
...
class CacheDataLoader(torch.utils.data.DataLoader):
    def __init__(
        ...
        if distributed is True and self.max_cache_num != 0:
            ...
            print("#{} distributed indices (total: {})".format(os.getpid(), len(self.indices)))
    ...
    def __iter__(self):
        ...
        if self.max_cache_num < 0:
            ...
            assert len(self.cached) >= self.batch_size

            if self.batch_size == 1:
                for i in indices:
                    yield self.cached[i]
            elif self.batch_size > 1:
                for i in range(0, len(indices), self.batch_size):  # Iterate in steps of batch_size
                    batch_indices = indices[i:i+self.batch_size]
                    if len(batch_indices) < self.batch_size:
                            remaining = self.batch_size - len(batch_indices)
                            batch_indices += indices[:remaining]
                    batch = [self.cached[idx] for idx in batch_indices]
                    yield batch
            else:
                raise ValueError(f"Invalid batch_size: {self.batch_size}. batch_size must be a positive integer.")
        else:
            if self.shuffle is True:
                indices = torch.randperm(len(self.indices), generator=self.generator).tolist()  # shuffle for each epoch
                # print("#{} 1st index: {}".format(os.getpid(), indices[0]))
                ...
                if self.async_caching:
                    while True:
                            ...
                            if self.batch_size == 1:
                                ...
                                while len(cached) < self.batch_size:
                                    cached += self.cache_output_queue.get()
                                for i in range(0, len(cached), self.batch_size):
                                    batch = cached[i:i+self.batch_size]
                                    if len(batch) < self.batch_size:
                                        remaining = self.batch_size - len(batch)
                                        batch += cached[:remaining]
                                    yield batch
                            else:
                                raise ValueError(f"Invalid batch_size: {self.batch_size}. batch_size must be a positive integer.")
                else:
                    assert self.max_cache_num >= self.batch_size
...
class DataModule(LightningDataModule):
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        return CacheDataLoader(
            ...
            seed=torch.initial_seed(),  # same seed like grendel-gs
    ...
    def on_after_batch_transfer(self, batch: Any, dataloader_idx: int) -> Any:
        ...
        for item in batch:
            if item[1][1] is None or item[1][1].dtype != torch.uint8:
                items.append(item)
            else:
                ...
                items.append((camera, (image_name, gt_image, masked_pixels), extra_data))
